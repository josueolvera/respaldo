...
public class CZonasController {
    ...
    @RequestMapping(value = "/save-zonas", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<String> save(@RequestBody String data, HttpSession session)throws IOException{
        JsonNode node = mapper.readTree(data);
        Users user = (Users) session.getAttribute("user");
        CZonas cZonas = new CZonas();
        cZonas.setName(node.get("name").asText());
        cZonas.setSaemFlag(0);
        cZonas.setCreationDate(LocalDateTime.now());
        cZonas.setUsername(user.getUsername());

        zonasService.save(cZonas);

        return new ResponseEntity<>(mapper.writerWithView(JsonViews.Embedded.class).writeValueAsString(zonasService.findAll())
                ,HttpStatus.OK);
    }
}
