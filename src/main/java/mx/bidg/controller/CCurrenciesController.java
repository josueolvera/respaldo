...
public class CCurrenciesController {
    ...
    @RequestMapping(value = "/{idCurrency}", method = RequestMethod.GET, produces = {"application/json;charset=UTF-8"})
    public @ResponseBody ResponseEntity<String> getById(@PathVariable int idCurrency) throws Exception {
        String response = mapper.writerWithView(JsonViews.Root.class).writeValueAsString(currenciesService.geById(idCurrency));
